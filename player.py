...
class Player(Entity):
    def __init__(self,pos,groups,obstacle_sprites, create_attack, destroy_attack, create_magic, trigger_death_particles):
        ...
        self.heal_duration_cooldown = 20
        self.heal_time = None
        self.can_heal = True
        ...
        # import a sound
        self.weapon_attack_sound = pygame.mixer.Sound(sound_effect_data['attack'])
        ...
        if not self.attacking:
            ...
            if self.can_switch_weapon:
                if keys[pygame.K_q]:
                    ...
                    if len(list(weapon_data.keys()))-1 > self.weapon_index:
                        self.weapon_index += 1
                    else:
                        self.weapon_index = 0
                    self.weapon = list(weapon_data.keys())[self.weapon_index]
            ...
            if keys[pygame.K_TAB] and self.can_switch_magic:       
                ...
                if len(list(magic_data.keys()))-1 > self.magic_index:
                    self.magic_index += 1
                else:
                    self.magic_index = 0
    ...
    def cooldowns(self):
        ...
        if not self.can_heal:
            if current_time - self.heal_time >= self.heal_duration_cooldown:
                self.can_heal = True
