...
class MagicPlayer:
    def __init__(self, animation_player):
        ...
        self.sounds = {
            'heal': pygame.mixer.Sound(sound_effect_data['heal']),
            'flame': pygame.mixer.Sound(sound_effect_data['flame'])
        }
        ...
        if player.energy >= cost:
            ...
            for i in range(1,6):
                if direction.x: #horizontal
                    ...
                    y = player.rect.centery + offset_y * randint(0,1) + randint(-TILESIZE // 3, TILESIZE //3)
                    self.animation_player.create_particles('flame', (x,y), groups)
